-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/arotti/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/arotti/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/arotti/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/arotti/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/arotti/outputs' USING PigStorage('\t');
